...
| **otel** | Configurable OpenTelemetry integration | `otel_initialize(*, context=None, instrumentors=None, log_filter=None)`, `OTelSettings` (env-prefix configurable), `default_otel_instrumentors()`, `instrument_fastapi(app)`; the OTLP log sink uses `{PREFIX}LOG_LEVEL` and `log_filter` |
...
| **boot** | Application / library boot sequence | `boot(context, sentry_integrations, otel_instrumentors, log_filter, show_cmdline)` — runs once per process: parses `--env` CLI args, initialises logging, Sentry and OpenTelemetry (`log_filter` applies to the stderr, file and OTLP log sinks), amends the SSL trust chain via *truststore* and *certifi*, and logs boot/shutdown messages                                                                                                                                                                                                                                                                                                                                                                                                |
...
- **Key Features**:
  - `boot(context, sentry_integrations, otel_instrumentors, log_filter, show_cmdline)` — runs once per process; subsequent calls are silent no-ops
  ...
  - Calls `otel_initialize(context, instrumentors, log_filter)`; the same `log_filter` goes to `logging_initialize` and to the OTLP log sink
...
- **Dependencies**: `sentry-sdk>=2.68.0,<3` (mandatory); `loguru>=0.7,<1`
- **Import**: `from aignostics_foundry_core.sentry import SentrySettings, sentry_initialize, set_sentry_user`

### otel

**Configurable OpenTelemetry integration for traces, metrics and logs**

- **Purpose**: Sets up the process-wide OTLP/gRPC `TracerProvider`, `MeterProvider` and `LoggerProvider`, each gated by its own setting, and bridges loguru records into OTLP logs.
- **Key Features**:
  - `OTelSettings(OpaqueSettings)` — env prefix `{ctx.env_prefix}OTEL_`. Fields: `enabled` (default `False`), `traces_enabled` (default `True`), `metrics_enabled` (default `True`), `logs_enabled` (default `False`). Endpoint, service name and exporter options come from the standard unprefixed `OTEL_*` env vars
  - `otel_initialize(*, context=None, instrumentors=None, log_filter=None)` — returns `False` unless enabled and `OTEL_EXPORTER_OTLP_ENDPOINT` is set; applies `instrumentors` (default `default_otel_instrumentors()`) when traces are on
  - OTLP log sink: uses `LogSettings.level` (`{PREFIX}LOG_LEVEL`) and drops records that `log_filter` rejects, the same as the stderr and file sinks; also drops the self-diagnostics of `opentelemetry` and `grpc`
  - `instrument_fastapi(app)` — request-level tracing for a FastAPI app; `init_api` calls it
- **Location**: `aignostics_foundry_core/otel.py`
- **Dependencies**: `opentelemetry-sdk`, `opentelemetry-exporter-otlp-proto-grpc`, OTel instrumentations for FastAPI, httpx and SQLAlchemy, `opentelemetry-resourcedetector-gcp` (all mandatory); `aignostics_foundry_core.log`
- **Import**: `from aignostics_foundry_core.otel import OTelSettings, otel_initialize, instrument_fastapi`
